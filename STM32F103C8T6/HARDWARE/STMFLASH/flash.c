...
U32 addrShort3 = 0x0800C000 + 12 * 16; // 2
U32 addrShort4 = 0x0800C000 + 13 * 16; // 2
void W_Test(void)
{

  System_WriteInteriorFlash(addrShort1, (uint8_t *)&threshold_value_init.ls_v, sizeof(threshold_value_init.ls_v)); // 存入数据，函数内自动解锁，擦除，写入
	System_WriteInteriorFlash(addrShort2, (uint8_t *)&threshold_value_init.lm_v, sizeof(threshold_value_init.lm_v)); // 存入数据，函数内自动解锁，擦除，写入
	System_WriteInteriorFlash(addrShort3, (uint8_t *)&Data_init.ls, sizeof(Data_init.ls)); // 存入数据，函数内自动解锁，擦除，写入
	System_WriteInteriorFlash(addrShort4, (uint8_t *)&Data_init.lm, sizeof(Data_init.lm));
}
// 读取数据
void R_Test(void)
{
  System_ReadInteriorFlash(addrShort1, (uint8_t *)&threshold_value_init.ls_v, sizeof(threshold_value_init.ls_v)); // 读取数据，函数内自动按半字读取
  System_ReadInteriorFlash(addrShort2, (uint8_t *)&threshold_value_init.lm_v, sizeof(threshold_value_init.lm_v)); // 读取数据，函数内自动按半字读取
  System_ReadInteriorFlash(addrShort3, (uint8_t *)&Data_init.ls, sizeof(Data_init.ls)); // 读取数据，函数内自动按半字读取
  System_ReadInteriorFlash(addrShort4, (uint8_t *)&Data_init.lm, sizeof(Data_init.lm));
}
/*********************************************************************************
